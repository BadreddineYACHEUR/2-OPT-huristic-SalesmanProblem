...
public class TwoOpt {
    ...
    public static ArrayList<Integer> alternate(ArrayList<Integer> cities, float[][] cities_distance) {
        ArrayList<Integer> newTour;
        double bestDist = Length.routeLength(cities, cities_distance);
        double newDist;
        int swaps = 1;
        int improve = 0;
        int iterations = 0;
        ...
        while (swaps != 0) { //loop until no improvements are made.
            ...
            for (int i = 1; i < cities.size() - 2; i++) {
                for (int j = i + 1; j < cities.size() - 1; j++) {
                    comparisons++;
                    //check distance of line A,B + line C,D against A,C + B,D if there is improvement, call swap method.
                    if(cities_distance[Math.min(cities.get(i), cities.get(i-1))][Math.max(cities.get(i), cities.get(i-1))]+
                            cities_distance[Math.min(cities.get(j), cities.get(j+1))][Math.max(cities.get(j), cities.get(j+1))]
                            >=
                            cities_distance[Math.min(cities.get(i), cities.get(j+1))][Math.max(cities.get(i), cities.get(j+1))]+
                            cities_distance[Math.min(cities.get(j), cities.get(i-1))][Math.max(cities.get(j), cities.get(i-1))])
                     {

                        newTour = swap_(cities, i, j); //pass arraylist and 2 points to be swapped.

                        newDist = Length.routeLength(newTour, cities_distance);
    ...
    private static ArrayList<Integer> swap_(ArrayList<Integer> cities, int i, int j) {
        //conducts a 2 opt swap by inverting the order of the points between i and j
